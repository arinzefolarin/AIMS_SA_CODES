...
class DecayingEpsilonGreedyPolicy2(Policy):
    """
    The Epsilon-Greedy policy will choose a random action with probability
    epsilon and take the best apparent approach with probability 1-epsilon. If
    multiple actions are tied for best choice, then a random action from that
        ...
        if evaluate:
            ...
            if np.random.random() < self.epsilon:
                ...
                if len(check) == 1:
                    ...
                    return np.random.choice(check)
class DecayingEpsilonGreedyPolicy3(Policy):
    """
    The Epsilon-Greedy policy will choose a random action with probability
    epsilon and take the best apparent approach with probability 1-epsilon. If
    multiple actions are tied for best choice, then a random action from that
    ...
    def choose(self, agent,evaluate=False):
        if evaluate:
            ...
            if self.total_time>0:
                ...
                P_end = 0.01
                print(self.P_init)
                epsilon = (self.P_init - P_end)* r + P_end
                self.P_init = epsilon
...
class GradientBanditPolicy(Policy):
    ...
    def choose(self, agent,evaluate=False):
        if evaluate:
            ...
            return np.random.choice(agent.indices, p=self.action_prob) ## re
...
if __name__ == '__main__':
    ...
    start_value =[1.0,1.0,1.0]
    Decaying_policies = [DecayingEpsilonGreedyPolicy1,DecayingEpsilonGreedyPolicy2,DecayingEpsilonGreedyPolicy3]
    num_cores = os.cpu_count()
    
    data = dict()
    labels = []
    x=np.arange(num_episodes)
    for i, val in enumerate(start_value):
        agent_id = 'agent_'+str(i)
        data[agent_id] = {'decay_epsilon':val} 
        labels.append('decay_eps method'+str(i+1))
        agents = [MalariaAgent(environment=gym.make("ushiriki_policy_engine_library:ChallengeAction-v0", userID="61122946-1832-11ea-8d71-362b9e155667"),
                  policy=Decaying_policies[i](val),gamma=0.9,initial=0,prior=0, 
    ...
    plt.title("Training plot for decaying epsilon greedy (Context-Free)")
    plt.xlabel('episodes')
    plt.ylabel('average episodic reward')
    plt.legend(shadow=True,fancybox=True,framealpha=0.7,loc='best')
    #plt.show()
    plt.savefig('decaying e-greedy_contextfree.png')
    
    full_json_dir = "decaying_egreedy_contextfree.json"
