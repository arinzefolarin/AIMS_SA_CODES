...
class UCBPolicy(Policy):
    ...
    def choose(self, agent, evaluate=False):
        if evaluate:
            ...
            UCB_estimation = agent.value_estimates + self.c * np.sqrt(np.log(agent.t + 1) / (agent.action_attempts + 1e-5))
...
if __name__ == '__main__':
    ...
    plt.title("Training plot for UCB (Context-Free)")
    plt.xlabel('episodes')
    plt.ylabel('average episodic reward')
    plt.legend(shadow=True,fancybox=True,framealpha=0.7,loc="best")
    #plt.show()
    plt.savefig('ucb_ContextFree.png')
    
    full_json_dir = "UCB_contextfree.json"
