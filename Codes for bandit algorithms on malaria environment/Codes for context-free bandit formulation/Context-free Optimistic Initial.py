...
if __name__ == '__main__':
    ...
    for i, init in enumerate(Initials):
        ...
        data[agent_id] = {'Initial':init} 
        labels.append('Initial Q ='+str(init))
        agents = [MalariaAgent(environment=gym.make("ushiriki_policy_engine_library:ChallengeAction-v0", userID="61122946-1832-11ea-8d71-362b9e155667"),
                  policy=GreedyPolicy(),gamma=0.9,initial=Initials[i],prior=0, 
    ...
    plt.title("Training plot for optimistic initial value (Context-Free)")
    plt.xlabel('episodes')
    plt.ylabel('average episodic reward')
    plt.legend(shadow=True,fancybox=True,framealpha=0.7,loc='best')
    #plt.show()
    plt.savefig('Optimistic_Initial_ContextFree.png')
    
    full_json_dir = "Optimistic_Initial_ContextFree.json"
