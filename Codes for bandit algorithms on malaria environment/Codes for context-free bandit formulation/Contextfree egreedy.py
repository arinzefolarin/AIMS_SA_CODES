...
if __name__ == '__main__':
    ...
    for i, eps in enumerate(epsilons):
        ...
        data[agent_id] = {'epsilon':eps} 
        labels.append('eps='+str(eps))
        agents = [MalariaAgent(environment=gym.make("ushiriki_policy_engine_library:ChallengeAction-v0", userID="61122946-1832-11ea-8d71-362b9e155667"),
                  policy=EpsilonGreedyPolicy(eps),gamma=0.9,initial=0,prior=0, 
    ...
    plt.title("Training plot for epsilon-greedy (Context-Free)")
    plt.xlabel('episodes')
    plt.ylabel('average episodic reward')
    plt.legend(shadow=True,fancybox=True,framealpha=0.7,loc="best")
    #plt.show()
    plt.savefig('Epsgreedy_context-free.png')
    
    full_json_dir = "epsilon_greedy_context-free.json"
