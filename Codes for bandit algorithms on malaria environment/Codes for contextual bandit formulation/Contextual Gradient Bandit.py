...
class ContextualMalariaAgent:
    def __init__(self, environment, policy,initial=0,
        ...
        self.agent =[ GradientAgent(environment,
                           self.k_arms,
                           policy,initial,
                           prior,alpha,baseline) for _ in range(5)]
...
if __name__ == '__main__':
    ...
    #plt.title("Plot for GradientBandit (contextual)")
    plt.xlabel('episodes')
    plt.ylabel('average episodic reward')
    plt.legend(shadow=True,fancybox=True,framealpha=0.7,loc="best")
    #plt.show()
    plt.savefig('GradientBandit_contextual.png')
    
    full_json_dir = "GradientBandit_contextual.json"
